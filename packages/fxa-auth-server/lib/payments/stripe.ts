...
import {
  ...
  STRIPE_TAX_RATES_CACHE_KEY,
  SUBSCRIPTION_PROMOTION_CODE_METADATA_KEY,
...
export class StripeHelper extends StripeHelperShared {
  private stripeTaxRatesCacheTtlSeconds: number; // This doesn't appear to be used. Can we delete?
  ...
  /**
   * Given a stripe plan extended with a product that has metada, this routine checks to make sure
   * state of plan is valid.
   * @param plan plan to validate
   * @returns true if plan has valid state
   */
  protected async validatePlan(plan: Stripe.Plan) {
  ...
  async fetchCustomer(
    ...
    try {
      return await this._fetchCustomer(uid, expand);
    } catch (err) {
