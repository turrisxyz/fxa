import { FxaTag } from './models/FxaTag';
import { TagOpts } from './models/TagOpts';
...
export function tagCriticalEvent(data: any) {
  if (data && data.request && data.request.url) {
    ...
    if (
      ...
      setTag(data, 'fxa.endpoint.type', 'critical');
    }
  }
...
export function setTag(data: any, key: FxaTag, value: string) {
  data = data || {};
  data.tags = data.tags || {};
  data.tags[key] = value;
  return data;
...
export function tagSentryEvent(event: Event, opts: TagOpts) {
  if (opts.criticalEndpoints) {
    event = tagCriticalEvent(opts);
  }
  if (opts.name) {
    event = setTag(event, 'fxa.name', opts.name);
  }
  return event;
}
